...
#         pub_dates.append(book.pub_date.strftime('%Y-%m-%d'))
#     pub_dates = list(set(pub_dates))
#     # index_dt = pub_dates.index(dt.date()) + 1
#     index_dt = pub_dates.index(dt) + 1
#
#     paginator = Paginator(pub_dates, 1)
#     page = paginator.get_page(index_dt)
#     prev_page = str(paginator.get_page(index_dt - 1).object_list[0].strftime('%Y-%m-%d'))
#     next_page = str(paginator.get_page(index_dt + 1).object_list[0].strftime('%Y-%m-%d'))
#     # prev_page = str(paginator.get_page(index_dt - 1).object_list[0])
#     # next_page = str(paginator.get_page(index_dt + 1).object_list[0])
#     context = {
#         'books': books,
#         'pagi': True,
#         'page': page,
